...
const output = new URL('../../app/build/reports/study-preview/',import.meta.url);
...
try {
  for (const [width,height] of [[360,640],[390,780],[412,844]]) {
    ...
    await context.addInitScript(() => {
      ...
      const boards={}; window.__preview = { boards, requests:[], saved:'{}', settings:{nodes:4000,leelaEnabled:true}, errors:[] };
      ...
      window.InstinctaZeroNative = {
        listBoards:()=>JSON.stringify(Object.values(boards).map(({tree,...meta})=>meta)),
        readBoard:id=>JSON.stringify(boards[id]||{}),
        saveBoard:raw=>{const state=JSON.parse(raw);if(state.gameId)return false;boards[state.boardId]=state;return true;},
        deleteBoard:id=>{delete boards[id];return true;},
        importPgn:()=>{window.__preview.importRequested=true;},
        exportPgn:raw=>{window.__preview.exported=raw;},
        ...
        saveStudyState:raw => { window.__preview.saved=raw;const state=JSON.parse(raw);if(state.boardId&&!state.gameId)boards[state.boardId]=state;return true; },
        cancelAnalysis:id => canceled.add(id),
        leaveAnalysis:() => { window.__preview.left=true; },
        startAnalysis:raw => {
          const id = 'analysis-'+ ++sequence, chess = position(raw);
          window.__preview.requests.push({id,...JSON.parse(raw)});
          ...
          setTimeout(()=>{ if(!canceled.has(id))window.InstinctaZero.onNativeAnalysis(id,{event:'done',data:{}}); },60);
          return id;
        },
        requestExplorer:raw => {
    ...
    await page.waitForFunction(()=>window.__preview.requests.some(r=>r.nodes===200));
    // Wait for the short reply search to finish; it must not chain into more searches.
    await page.waitForTimeout(100);
    const requested=await page.evaluate(()=>window.__preview.requests.length);
    await page.waitForTimeout(350);
    assert.equal(await page.evaluate(()=>window.__preview.requests.length),requested);
    const shot = name=>page.screenshot({animations:'disabled',path:fileURLToPath(new URL(`${name}-${width}.png`,output))});
    await shot('engine');
    ...
    const instantArrows=await page.evaluate(()=>{document.querySelector('.pv').click();return document.querySelectorAll('.analysis-arrow').length;});
    assert.ok(instantArrows>1,'prepared replies must appear before the next network request');
    ...
    await page.locator('.pv').first().waitFor();
    await page.waitForFunction(()=>document.querySelectorAll('.pv').length>8);
    ...
    assert.deepEqual(await history(),[]);
    await page.getByRole('button',{name:'More',exact:true}).tap();
    await page.getByRole('button',{name:'Repertoire',exact:true}).tap();
    await page.getByRole('button',{name:'White',exact:true}).tap();
    await page.getByRole('button',{name:'More',exact:true}).tap();
    await page.getByRole('button',{name:'PGN files',exact:true}).tap();
    await page.getByRole('button',{name:'Export study',exact:true}).tap();
    assert.ok((await page.evaluate(()=>window.__preview.exported)).includes('1... c5'));
    await page.getByRole('button',{name:'Moves',exact:true}).tap();
    const e6=page.locator('.move').filter({hasText:'e6'});
    await e6.dispatchEvent('pointerdown',{pointerType:'touch',button:0});
    await page.getByRole('button',{name:'Promote variation · up one',exact:false}).waitFor();
    await shot('variation-actions');
    await page.getByRole('button',{name:'Promote variation · up one',exact:false}).tap();
    assert.deepEqual(await page.evaluate(()=>JSON.parse(window.__preview.saved).tree[0].c.map(n=>n.u)),['e7e5','e7e6','c7c5']);
    await nav('next');await nav('next');
    assert.deepEqual(await history(),['e2e4','e7e5']);
    await page.getByRole('button',{name:'More',exact:true}).tap();
    await page.getByRole('button',{name:'Studies / chapters',exact:true}).tap();
    await page.locator('.library-item').nth(1).tap();
    assert.deepEqual(await history(),[]);
    await nav('next');assert.deepEqual(await history(),['d2d4']);
    assert.ok(await page.locator('.repertoire-status').count());
    await shot('repertoire');
    await page.getByRole('button',{name:'Engine',exact:true}).tap();
    await page.locator('.pv').first().waitFor();
    await page.waitForTimeout(500);
    await page.evaluate(()=>{
      const request=window.__preview.requests.filter(r=>r.nodes===4000).at(-1);
      window.InstinctaZero.onNativeAnalysis(request.id,{event:'error',message:'Connection closed'});
    });
    await page.getByRole('button',{name:'Retry Leela',exact:false}).waitFor();
    const beforeRetry=await page.evaluate(()=>window.__preview.requests.length);
    await page.waitForTimeout(150);
    assert.equal(await page.evaluate(()=>window.__preview.requests.length),beforeRetry,'failed analysis is never silently replayed');
    await page.getByRole('button',{name:'Retry Leela',exact:false}).tap();
    await page.waitForFunction(count=>window.__preview.requests.length>count,beforeRetry);
