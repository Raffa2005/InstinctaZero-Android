...
(function () {
  ...
  window.InstinctaZeroPosition = {START,parse,fen,placement,canCastle,epOptions,changed,validate};

  window.createPositionEditor = function (api) {
    let state=parse(START),tool='move',page='pieces',black=false,opened=false,ground=null,undo=[],notice='',fenError=false;
    const el=document.createElement('section');el.id='position-editor';el.hidden=true;el.setAttribute('aria-label','Board editor');
    el.innerHTML='<header class="pe-head"><button data-pe-close aria-label="Cancel position editing">‹</button><strong>Board editor</strong><button data-pe-flip aria-label="Flip editor board">⇅</button></header><div class="pe-board-wrap cburnett orientation-white"><div class="board pe-board"></div><div class="pe-squares"></div></div><nav class="pe-tabs" aria-label="Editor tabs"><button data-pe-tab="pieces">Pieces</button><button data-pe-tab="state">State</button><button data-pe-tab="fen">FEN</button></nav><div class="pe-panel"></div><div class="pe-status" role="status"></div><footer class="pe-footer"><button data-pe-close>Cancel</button><button data-pe-analyse>Analyse position</button></footer>';
    ...
    function render(){
      ...
      squares.hidden=tool==='move';squares.innerHTML='';
      for(let i=0;i<64;i++){const file=black?7-i%8:i%8,rank=black?1+Math.floor(i/8):8-Math.floor(i/8),sq=String.fromCharCode(97+file)+rank,b=document.createElement('button');b.setAttribute('aria-label',sq);b.onclick=()=>{stash();if(tool==='erase')delete state.pieces[sq];else {if(tool==='K'||tool==='k')for(const key of Object.keys(state.pieces))if(state.pieces[key]===tool)delete state.pieces[key];state.pieces[sq]=tool;}changed(state);sync();};squares.appendChild(b);}
      if(page==='pieces') {
        panel.innerHTML='<div class="pe-palette">'+[...'KQRBNPkqrbnp'].map(p=>'<button data-pe-piece="'+p+'" aria-label="'+(p===p.toUpperCase()?'White ':'Black ')+roles[p.toLowerCase()]+'" class="'+(tool===p?'selected':'')+'"><img alt="" src="pieces/'+(p===p.toUpperCase()?'w':'b')+p.toUpperCase()+'.svg"></button>').join('')+'</div><div class="pe-tools"><button data-pe-tool="move">Move</button><button data-pe-tool="erase">Erase</button><button data-pe-undo>Undo</button><button data-pe-clear>Clear</button><button data-pe-reset>Reset</button></div>';
        panel.querySelectorAll('[data-pe-piece]').forEach(b=>b.onclick=()=>{tool=b.dataset.pePiece;render();});
        panel.querySelectorAll('[data-pe-tool]').forEach(b=>{b.classList.toggle('selected',b.dataset.peTool===tool);b.onclick=()=>{tool=b.dataset.peTool;render();};});
    ...
    function close(accepted){opened=false;el.hidden=true;api.close(accepted||null,black);}
    ...
    return {isOpen:()=>opened,close:()=>close(null),open:(currentFen,currentBlack,useCurrent=false)=>{
      ...
      if(!ground)ground=window.LegacyChessground(board,{fen:placement(state),coordinates:true,autoCastle:false,animation:{enabled:false},movable:{color:'both',free:true,showDests:false},premovable:{enabled:false},draggable:{enabled:true,magnified:false,deleteOnDropOff:true},events:{change:()=>{if(!opened)return;stash();state.pieces=parse(ground.getFen()+' '+state.turn+' - - 0 1').pieces;changed(state);sync();}}});
      sync();
    }};
  };
}());
